...
impl CameraObject for ArcBallCam {
    ...
    fn update(&mut self, ctx: &Context, is_active: bool) {
        ...
        //let offset_y = ctx.input.mouse.position_delta.1 * 1.0;
        //
        //let up = self.arc_transform.vector_to_world(&glm::Vec3::y());
        //let right = self.arc_transform.vector_to_world(&glm::Vec3::x());

        //if offset_x != 0.0 || offset_y != 0.0 {
        //    let new_pos = self.from_center + self.sensitivity * (offset_y * up + offset_x * right);
        ...
        // camera rotation
            let offset_x = ctx.input.mouse.position_delta.0 * -1.0;
            let offset_y = ctx.input.mouse.position_delta.1 * -1.0;
            self.camera.transform.rotate(self.from_center, offset_x * self.sensitivity * ctx.time.delta_time() as f32, Space::World);
            self.camera.transform.rotate(glm::Vec3::x(), offset_y * self.sensitivity * ctx.time.delta_time() as f32, Space::Local);
        }
    }
}
